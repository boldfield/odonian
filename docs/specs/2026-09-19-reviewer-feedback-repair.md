...
All three are sized for Haiku and start with Haiku implementation, with automatic escalation enabled (`escalate=true`), Opus and GPT-5.5 review, and human merge (`agent_merge=false`). The user clarified this policy on 2026-09-19: Haiku is the task-size target and initial model, not an escalation ceiling. This supersedes the original escalation-disabled wording, including copies in existing task specifications. Publish the approved corrective specification at a pinned repository commit before task creation, and reference it explicitly so the old approved spec cannot override the new contract.
...
Only after that verification, return the existing N1 replacement from blocked to ready with a diagnosis note, keeping its PR, acceptance criteria, dependencies, reviewers, and review history. Enable the task escalation policy in place; the configured escalation ladder may change the implementation model through Odonian's normal recorded supersession workflow. The worker must implement the missing regression coverage and pass normal review. The completed rejection history must remain intact; no manual reset of review history, ad hoc replacement ticket, forced approval, or direct referee-code patch is part of this repair. Normal Odonian model escalation remains permitted.
